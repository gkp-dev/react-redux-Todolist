...
export const TodoItem = ({todo,onToggle,onDelete})=>{
    ...
    <ul>
        <li className="my-4">
            <input className="mx-2" type="checkbox" onChange={()=>onToggle(todo)} checked={todo.complete}/>
            {todo.title}
            <button onClick={()=>onDelete(todo)} className="bg-red-500 p-2 rounded-lg text-white ml-2">X</button>
        </li>
    </ul>
    )
...
export const TodoList = ({todos,onToggle,onDelete})=>{
    return todos.map(todo=><TodoItem todo={todo} onToggle={onToggle} onDelete={onDelete}/>)
